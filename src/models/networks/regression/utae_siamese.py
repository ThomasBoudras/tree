...
from src.models.networks.regression.ltae import LTAE2d


class UTAE_siamese(nn.Module):
    def __init__(
        self,
        input_dim,
        ...
        """
        super(UTAE_siamese, self).__init__()
        ...
        d_skip_factor = 1 if self.coupling_mode == "difference" else 2
        self.up_blocks = nn.ModuleList(
            UpConvBlock_siamese(
                d_in=decoder_widths[i],
                d_out=decoder_widths[i - 1],
                d_skip=encoder_widths[i - 1]*d_skip_factor,
                k=str_conv_k,
                s=str_conv_s,
                p=str_conv_p,
                norm="batch",
    ...
    def forward(self, input, meta_data=None, return_att=False):
        batch_positions = meta_data["dates"]
        
        input_separation = input.shape[1]//2
        batch_positions_seperation = batch_positions.shape[2]//2

        input_1 = input[:, :input_separation,:, :]
        batch_positions_1 = batch_positions[:, :batch_positions_seperation,:, :]

        input_2 = input[:, input_separation:,:, :]
        batch_positions_2 = batch_positions[:, batch_positions_seperation:,:, :]
        
        # SPATIAL and TEMPORAL ENCODER
        out_1, att_1, feature_maps_1, pad_mask_1 = self.forward_encoder(input_1, batch_positions_1)
        out_2, att_2, feature_maps_2, pad_mask_2 = self.forward_encoder(input_2, batch_positions_2)
        ...
        # SPATIAL DECODER
        for i in range(self.n_stages - 1):
            skip_1 = self.temporal_aggregator(
                feature_maps_1[-(i + 2)], pad_mask=pad_mask_1, attn_mask=att_1
            )
            skip_2 = self.temporal_aggregator(
                feature_maps_2[-(i + 2)], pad_mask=pad_mask_2, attn_mask=att_2
            )
            out = self.up_blocks[i](out=out, skip_1=skip_1, skip_2=skip_2)
...
class UpConvBlock_siamese(nn.Module):
    def __init__(
        self, d_in, d_out, k, s, p, norm="batch", d_skip=None, padding_mode="reflect", coupling_mode= "concat"
    ):
        super(UpConvBlock_siamese, self).__init__()
        self.coupling_mode = coupling_mode
    ...
    def forward(self, input, skip_1, skip_2):
        out = self.up(input)

        if self.coupling_mode == "difference" :
            skip = self.skip_conv(skip_2) - self.skip_conv(skip_1)

        if self.coupling_mode == "concat" :
            skip = torch.cat([self.skip_conv(skip_1), self.skip_conv(skip_2)], dim=1)
