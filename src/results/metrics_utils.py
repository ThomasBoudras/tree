...
def get_mask(image_path, min_area=1000000, plot=True, plot_path="mask.jpg", crs=2154):
    with rasterio.open(image_path) as src:
        ...
        polygons = [
            ...
            for geom, value in tqdm(
                features.shapes(mask.astype(np.int16), transform=src.transform), desc="Retrieving mask"
            )
            if value == 1
        ]
...
def mask_with_geometry(tif_path, geometry, output_tif_path, nodata=np.nan):
    ...
    with rasterio.open(tif_path) as src:
        ...
        mask = rasterio.features.geometry_mask(
            [mapping(geometry)], transform=src.transform, invert=False, out_shape=src.shape
        )
        # Apply the mask to the TIFF data
        masked_tif_data = np.where(mask, tif_data, nodata)  # Using NaN to indicate masked areas
...
def compute_change_metrics(mask1, mask2, difference_1, difference_2, resolution=1.5):
    ...
    fn = np.sum(np.logical_and(mask1, np.logical_not(mask2)))  # False negatives
    ...
    # res["specificity"] = tn / (tn + fp) if tp + fp > 0 else 0
    # res["sensitivity"] = tp / (tp + fn) if tp + fp > 0 else 0
    res["iou"] = compute_iou(mask1, mask2)

    # Metrics between difference and difference pred
    res["difference_mae"] = np.mean(np.abs(difference_1 - difference_2))
    res["difference_rmse"] = np.sqrt(np.mean((difference_1 - difference_2) ** 2))
    res["difference_mean_error"] = np.mean(difference_1 - difference_2)

    # Growth on whole area
    res["growth_ref"] = np.mean(difference_1)
    res["growth_pred"] = np.mean(difference_2)

    # Area change
    res["area_change_ref"] = mask1.sum() * (resolution**2)
    res["area_change_pred"] = mask2.sum() * (resolution**2)
    res["area_change_diff"] = res["area_change_pred"] - res["area_change_ref"]
    res["area_change_diff_norm"] = res["area_change_diff"] / res["area_change_ref"]

    # Volume change = integration of difference on mask
    res["volume_change_ref"] = (difference_1 * mask1).sum() * (resolution**2)
    res["volume_change_pred"] = (difference_2 * mask2).sum() * (resolution**2)
    res["volume_change_diff"] = res["volume_change_pred"] - res["volume_change_ref"]
    res["volume_change_diff_norm"] = res["volume_change_diff"] / res["volume_change_ref"]

    # Volume change wo mask = integration of difference
    res["volume_change_wo_mask_ref"] = difference_1.sum() * (resolution**2)
    res["volume_change_wo_mask_pred"] = difference_2.sum() * (resolution**2)
    res["volume_change_wo_mask_diff"] = res["volume_change_wo_mask_pred"] - res["volume_change_wo_mask_ref"]
    res["volume_change_wo_mask_diff_norm"] = (
        res["volume_change_wo_mask_diff"] / res["volume_change_wo_mask_ref"]
    )

    # mae on mask1
    res["mae_on_detection_mask"] = np.mean(np.abs(difference_1[mask1] - difference_2[mask1]))
    res["rmse_on_detection_mask"] = np.sqrt(np.mean((difference_1[mask1] - difference_2[mask1]) ** 2))
...
def plot_detections(
    ...
    image_2 = image_2[[2,1,0],:,:].astype(np.float32)
    ...
    axes[0, 1].set_title(f"Sentinel {year_2}")
    ...
    axes[2, 2].imshow(metrics_out["difference_pred"], cmap=cmap_diff, norm=norm)
